...
We fine-tuned Qwen2.5-7B-Instruct on a single A100 via HF AutoTrain. LoRA adapter (r=16), 4-bit base model, 5 epochs, 21 minutes wall-time.

![SFT training loss](demo/training_loss.png)

*Loss decreases 8× across 45 SGD steps. Final loss 0.53 indicates strong fit on the imitation target.*

![Terminal reward by model](demo/before_after_chart.png)

*Mean terminal reward across all rollouts. SFT closes ~95% of the gap to Opus on this task.*
...
*Where the reward comes from. The SFT'd model locks in 5 of 6 keyword-rubric dimensions, missing only `avoids_global_rollback` — the same ones a frontier model gets right.*

![Step reward accumulation](demo/step_reward_curve.png)

*Watch one episode play out. The SFT'd model earns step-level rewards as it correctly progresses through the investigation, then commits to the right submit at step 18. The baseline parses-fails at step 1 and earns nothing.*
...
- **No GRPO yet.** Our research showed every successful OpenEnv RL example to date is on much simpler tasks (BlackJack, Wordle, 2048). LinkedIn's GPT-OSS multi-tool RL retro used 16 H100 nodes and ran for days, with multiple silent failure modes (MoE log-prob mismatches, FA2/FA3 attention-sink kernel bugs). That's not a hackathon-timeline experiment. SFT first; we ship a runnable GRPO recipe at `colab/cloud_sec_env_grpo.ipynb` that wires our env to TRL's `GRPOTrainer` — it's a recipe, not a result.
...
- **Only one task.** The framework supports adding tasks 2-10 by dropping fixtures into `cloud_sec_env/data/`. We chose depth over breadth for the hackathon.
...
- **Reproduce the baseline**: `pip install openenv-core` then point an `EnvClient` at the Space URL.
- **Fine-tune**: open `colab/cloud_sec_env_sft.ipynb` in Colab, set runtime to T4, hit Run All. Self-contained.
- **Eval the SFT'd adapter**: open `colab/cloud_sec_env_eval_sft.ipynb` — loads our cleaned LoRA from the Hub and runs greedy rollouts against the live Space.
- **Code**: https://github.com/mohit24bcs10222/cloud-sec-env
...
If you're building agentic RL environments, three things from this project we'd encourage you to steal:
...
2. **Falsification rewards.** Reward not just naming the right hypothesis, but explicitly ruling out the alternative — frontier models miss this by default.
3. **Composable rubric.** Deterministic primary, LLM-judge auxiliary. Keeps reproducibility and gets you nuanced eval simultaneously.

---

## Acknowledgements

Built in 48 hours for the **Meta-PyTorch / HuggingFace OpenEnv hackathon (April 2026)**. Thanks to Meta-PyTorch for OpenEnv, HuggingFace for hosting + AutoTrain, and the Unsloth team for making the LoRA training pattern accessible.
